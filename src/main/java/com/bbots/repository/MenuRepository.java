...
public class MenuRepository {
    ...
    private final RowMapper<Menu001> mapper001 = (rs, rowNum) -> new Menu001(
            rs.getString("PGM_ID"), rs.getString("DESCN"), rs.getInt("MODULE"),
    ...
    private final RowMapper<Menu002> mapper002 = (rs, rowNum) -> new Menu002(
            rs.getInt("ROLECD"), rs.getInt("Menucode"), rs.getString("MENU_DESCN"),
    ...
    private final RowMapper<Menu003> mapper003 = (rs, rowNum) -> new Menu003(
            rs.getInt("ROLECD"), rs.getInt("Menucode"), rs.getInt("submenucode"),
    ...
    public void saveProgram(Menu001 m) {
        String sql = "INSERT INTO MENU001 (PGM_ID, DESCN, MODULE, SUB_MODULE, PGM_CLASS, status, REMARKS, EUSER) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        jdbcTemplate.update(sql, m.getPgmId(), m.getDescn(), m.getModule(), m.getSubModule(), m.getPgmClass(), m.getStatus(), m.getRemarks(), m.getEUser());
    }
    ...
    public void saveParentMenu(Menu002 m) {
        String sql = "INSERT INTO MENU002 (ROLECD, Menucode, MENU_DESCN, MENU_ORDER, SUBMENUREQ, PARENTMENU_PGMID, \"Program Path\", MENU_LOGO, MENU_LOCATION, MENU_STATUS, EUSER) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        jdbcTemplate.update(sql, m.getRoleCd(), m.getMenuCode(), m.getMenuDescn(), m.getMenuOrder(), m.getSubMenuReq(), m.getParentMenuPgmId(), m.getProgramPath(), m.getMenuLogo(), m.getMenuLocation(), m.getMenuStatus(), m.getEUser());
    }
    ...
    public void saveSubMenu(Menu003 m) {
        String sql = "INSERT INTO MENU003 (ROLECD, Menucode, submenucode, Description, MENU_ORDER, SUBMENU_PGMID, \"Program Path\", MENU_LOGO, MENU_STATUS, EUSER) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        jdbcTemplate.update(sql, m.getRoleCd(), m.getMenuCode(), m.getSubMenuCode(), m.getDescription(), m.getMenuOrder(), m.getSubMenuPgmId(), m.getProgramPath(), m.getMenuLogo(), m.getMenuStatus(), m.getEUser());
    }
    ...
    public void saveMenuProgram(Menu004 m) {
        String sql = "INSERT INTO MENU004 (ROLECD, Menucode, submenucode, PGM_ID, Decription, MENU_ORDER, \"Program Path\", MENU_LOGO, status, EUSER) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        jdbcTemplate.update(sql, m.getRoleCd(), m.getMenuCode(), m.getSubMenuCode(), m.getPgmId(), m.getDescription(), m.getMenuOrder(), m.getProgramPath(), m.getMenuLogo(), m.getStatus(), m.getEUser());
    }
}
